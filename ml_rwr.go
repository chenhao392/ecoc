	...
	for j := 0; j < nPriorLabel; j++ {
		...
		for i := 0; i < nPriorGene; i++ {
			...
			if priorData.At(i, j) > 0 && exist && existTr {
				ind[j] += 1
				//inGene[idArr[i]] = idIdx[idArr[i]]
				inGene = append(inGene, idIdx[idArr[i]])
			}
		}
		//single
		//aupr better than base line?
		go singleIndFeature(network, nNetworkGene, nPriorGene, ind, j, idIdx, idArr, inGene, trGeneMap, priorData)
	}
	wg.Wait()
...
func singleIndFeature(network *mat64.Dense, nNetworkGene int, nPriorGene int, ind []int, j int, idIdx map[string]int, idArr []string, inGene []int, trGeneMap map[string]int, priorData *mat64.Dense) {
	defer wg.Done()
	if ind[j] > 1 {
		nCv := 2
		...
		for i := 0; i < nCv; i++ {
			...
			for k := 0; k < nPriorGene; k++ {
				...
				if priorData.At(k, j) > 0 && exist && existTr {
