...
func main() {
	...
	//tsY_prob := make([nROwTsY][nLabel]float64)
	//tsY_prob := [][]float64{}
	//for i := 0; i < nLabel; i++ {
	for i := 0; i < 2; i++ {
		//LR, regulazor, err := adaptiveTrainLGR_Liblin(trXdata, trYdata.ColView(i), 5, nFea)
		_, reg, err := adaptiveTrainLGR_Liblin(trXdata, trYdata.ColView(i), 5, nFea)
		fmt.Println("trained", reg, err)
	}

}
func adaptiveTrainLGR_Liblin(X *mat64.Dense, Y *mat64.Vector, nFold int, nFeature int) (LRmodel *linear.Model, regulator float64, errFinal float64) {
	//prior := make([]float64, nFeature)
	//lamda := [5]float64{0.01, 0.1, 0, 1, 10}
	...
	for i := 0; i < len(lamda); i++ {
		for j := 0; j < nFold; j++ {
			...
			//nr_feature := LRmodel.Nfeature()
			//fmt.Println(nr_feature)
			//LRmodel.bias = -1
			w := []float64{-1}
			//w2 := LRmodel.W()
			w = append(w, LRmodel.W()...)
			//LRmodel.w[2:LRmodel.nr_feature] = LRmodel.w[1 : LRmodel.nr_feature-1]
			//LRmodel.w[1] = b
	...
	//w = append(w, LRmodel.W()...)
	errFinal = err[idx]
	return LRmodel, regulator, errFinal
}
